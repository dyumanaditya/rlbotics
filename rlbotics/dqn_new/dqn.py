...
from rlbotics.dqn_new.replay_buffer import *
...
class DQN:
    ...
    def _build_policy(self):
        self.policy = MLPEpsilonGreedy(IO_sizes=[self.obs_dim, self.act_dim],
                                       hidden_sizes=h.hidden_sizes,
                                       layer_types=h.layer_types,
                                       activations=h.activations,
                                       optimizer=h.optimizer,
                                       lr=h.lr)

        self.target_policy = MLP(IO_sizes=[self.obs_dim, self.act_dim],
                                 hidden_sizes=h.hidden_sizes,
                                 layer_types=h.layer_types,
                                 activations=h.activations,
                                 optimizer=h.optimizer,
